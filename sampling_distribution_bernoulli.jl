...
#> risky_file_source = "https://github.com/marcus-waldman/CCHS-Statistical-Analysis-Spr.-2023-/blob/main/sampling_distribution_beta.jl?raw=true"
...
@bind N Slider([2,3,4,5,6,7,8,9,10,20,30,50,100,500,2000], 2, true)

# ╔═╡ fd8dfdaf-cc85-4fec-b3f6-0c52343284ff
md"""Probabilty of 'success', $p$"""

# ╔═╡ e59a02f9-b814-47a4-aba3-8a46cb0a14dc
@bind prob Slider(0:.01:1,.5,true)
...
# ╔═╡ e8267875-6e2e-4196-8f8b-627a96dc30ca
begin
	Draws = Int(round(draws))
	
	# Bernoulli distribution distribution
	dbern = Distributions.Binomial(1,prob);
	
	df = DataFrames.DataFrame(y = Distributions.rand(dbern, Draws))
	p1 = plot(
	    df, x = :y, 
	    Geom.histogram(density = true, bincount=2, limits = (min=0,max=1), position=:dodge), 
	    Guide.yticks(ticks=nothing), 
	    Guide.xticks(ticks = 0:1:1), 
	...
	ybar = [mean(Distributions.rand(dbern,N)) for i = 1:Draws];
	bardf = DataFrames.DataFrame(ybar = ybar);
	p2a = layer(
	    bardf, 
	    x = ybar,  Geom.histogram(density = true, bincount = nothing, position = :dodge), 
	    alpha = [0.3]
	);
	
	
	dnorm = Distributions.Normal(prob, sqrt(prob*(1-prob))/sqrt(N));
	p2b = layer(df, 
...
# ╔═╡ 6fe0868e-31d6-41d6-a220-bacd08628f07
p

# ╔═╡ 0bf0079d-cb5f-4e7e-aab6-41127c7160dd
txt0

# ╔═╡ 84313d0b-c7b2-48e5-84d4-f9b647a399b0
txt1

# ╔═╡ 3cd30911-ed89-4384-a806-0ea6b34aaa54
md"""Preview of data from Simulation 1"""

# ╔═╡ d77bb673-e3a2-49e0-84e2-1aac06f9a833
DataFrame(Data=Distributions.rand(dbern,N))

# ╔═╡ c29c18d5-b2cc-4eec-9eb4-36b5a679a963
md"""Sample means from all simulations."""

# ╔═╡ 12f0b3fb-28a6-417a-a2cd-84b488f743c1
DataFrame(Sample_Means = ybar)
